...
<p align="center"><strong>Proxy-skill для GPT Image API, ориентированный на Codex (OpenCode, Cursor и другие следует проверять самостоятельно), либо для других моделей в зависимости от интерфейса стороннего прокси. Позволяет создавать и редактировать изображения, поддерживает настраиваемый шлюз, параметры модели и локальный вывод изображений.</strong></p>
...
**image-gen-proxy** — это skill генерации изображений для AI-агентов и npm-установщик для OpenAI-совместимых прокси GPT Image API. Он не использует фиксированный gateway; `GPT_IMAGE_BASE_URL` и `GPT_IMAGE_API_KEY` читаются из `.env` или системного окружения.

| Skill | Описание | Модель по умолчанию |
|---|---|---|
| **Image Gen Proxy** | Text-to-image, редактирование, пакетная генерация, настраиваемый proxy, локальные загрузки | `gpt-image-2` |
