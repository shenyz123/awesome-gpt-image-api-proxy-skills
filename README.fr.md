...
<p align="center"><strong>Skill de proxy GPT Image API pour Codex (OpenCode, Cursor et autres doivent être vérifiés séparément), ou pour d'autres modèles selon l'interface fournie par le proxy tiers. Il permet de générer et modifier des images, avec passerelle configurable, paramètres de modèle et sortie d'images locale.</strong></p>
...
**image-gen-proxy** est un skill de génération d’images pour agents IA et un installateur npm pour les proxies GPT Image compatibles OpenAI. Il n’utilise pas de passerelle fixe ; `GPT_IMAGE_BASE_URL` et `GPT_IMAGE_API_KEY` sont lus depuis `.env` ou l’environnement système.

| Skill | Description | Modèle par défaut |
|---|---|---|
| **Image Gen Proxy** | Texte vers image, édition, génération par lot, proxy configurable, téléchargements locaux | `gpt-image-2` |
